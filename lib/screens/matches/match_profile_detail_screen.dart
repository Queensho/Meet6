...
class _MatchProfileDetailScreenState extends State<MatchProfileDetailScreen> {
  ...
  Color get page =>
      dark ? const Color(0xFF071022) : const Color(0xFFF9FAFE);
  Color get surface => dark ? const Color(0xFF111B2E) : Colors.white;
  Color get surface2 =>
      dark ? const Color(0xFF172338) : const Color(0xFFF4F5F9);
  Color get ink => dark ? const Color(0xFFF5F7FF) : AppColors.navy;
  Color get muted =>
      dark ? const Color(0xFFAEB8CC) : const Color(0xFF6E768C);
  Color get divider =>
      dark ? const Color(0xFF29364D) : const Color(0xFFE5E8F0);
  ...
  Future<void> _load() async {
    try {
      ...
      if (uid.isNotEmpty) {
        try {
          final d = await GiftService.userSummary(uid);
          if (d['summary'] is Map) {
  ...
  String get cityText {
    final value = profile?['city']?.toString().trim() ?? '';
    return value.isNotEmpty ? value : locationText;
  }

  String get genderText {
    final value = profile?['gender']?.toString().trim().toLowerCase() ?? '';
    if (['male', 'erkek', 'man'].contains(value)) return 'Erkek';
    if (['female', 'kadın', 'kadin', 'woman'].contains(value)) return 'Kadın';
  ...
  Future<bool?> _confirm(String title, String body) {
    return showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(body, style: TextStyle(color: muted)),
        actions: [
          TextButton(
  ...
  Future<void> _block() async {
    if (await _confirm(
              ...
              'Bu eşleşme kapanır ve bu kişiyle tekrar eşleşmezsin.',
            ) !=
            true ||
        userId.isEmpty) {
      return;
  ...
  Future<void> _report() async {
    ...
    await LiveService.reportUser(
      ...
      detail: 'Profil detayından bildirildi',
    );
    if (mounted) {
  ...
  Widget _networkPhoto(String url, {BoxFit fit = BoxFit.cover}) {
    return Image.network(
      ...
      errorBuilder: (_, __, ___) => Container(
        ...
        child: Icon(Icons.person_rounded, size: 72, color: ink),
      ),
    );
  }
          ...
          child: ClipRRect(
            ...
            child: photos.isEmpty
                ...
                : PageView.builder(
                    ...
                    onPageChanged: (index) =>
                        setState(() => photoIndex = index),
                    itemBuilder: (_, index) => _networkPhoto(photos[index]),
                  ),
          ),
        ),
        if (photos.length > 1)
          ...
          child: ListView.separated(
            ...
            itemBuilder: (_, index) => GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                width: 72,
                padding: EdgeInsets.all(index == photoIndex ? 3 : 0),
                decoration: BoxDecoration(
                  borderRadius: BorderRadius.circular(18),
                  border: index == photoIndex
                      ? Border.all(color: AppColors.lime, width: 3)
                      : null,
                ),
                child: ClipRRect(
                  borderRadius: BorderRadius.circular(14),
                  child: _networkPhoto(photos[index]),
                ),
              ),
            ),
          ),
  ...
  Widget _aboutAndInterests(int? age, String bio) {
    return _CardShell(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _sectionTitle(Icons.article_outlined, 'Hakkında'),
          ...
          if (interests.isNotEmpty) ...[
            ...
            _sectionTitle(Icons.interests_rounded, 'İlgi alanları'),
            const SizedBox(height: 13),
            Wrap(
              spacing: 8,
              runSpacing: 8,
              children: interests.map((e) => _TextChip(text: e)).toList(),
            ),
          ],
          const SizedBox(height: 20),
          Divider(height: 1, color: divider),
          const SizedBox(height: 18),
          _sectionTitle(Icons.person_outline_rounded, 'Profil bilgileri'),
          const SizedBox(height: 13),
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: [
              if (age != null)
                _InfoChip(icon: Icons.cake_outlined, text: '$age yaş'),
              ...
              if ((profile?['country']?.toString().trim() ?? '').isNotEmpty)
                _InfoChip(
  ...
  Widget _profileQuestion(String prompt, String answer) {
    return _CardShell(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _sectionTitle(Icons.chat_bubble_outline_rounded, 'Profil sorum'),
          const SizedBox(height: 14),
                    ...
                    children: [
                      Positioned(
                        ...
                        child: _Blob(
                          size: 220,
                          color: dark
                              ? const Color(0x243C5A22)
                              : const Color(0x1ABFFF24),
                        ),
                      ),
                      Positioned(
                        top: 360,
                        ...
                        child: _Blob(
                          size: 210,
                          color: dark
                              ? const Color(0x182F6BFF)
                              : const Color(0x10BFFF24),
                        ),
                      ),
                      ...
                      Positioned(
                        ...
                        child: SafeArea(
                          top: false,
                          child: _MessageButton(onTap: _message),
                        ),
                      ),
                    ],
                  ),
...
class _CircleButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Material(
      color: dark ? const Color(0xFF172338) : Colors.white,
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(28),
        border: Border.all(
          color: dark ? const Color(0xFF29364D) : const Color(0xFFE5E8F0),
        ),
        boxShadow: dark
            ? const []
            : const [
                BoxShadow(
                  color: Color(0x0B0B1745),
                  blurRadius: 18,
                  offset: Offset(0, 7),
                ),
              ],
...
class _InfoChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final dark = Theme.of(context).brightness == Brightness.dark;
    final ink = dark ? const Color(0xFFF5F7FF) : AppColors.navy;
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 9),
      decoration: BoxDecoration(
        color: dark ? const Color(0xFF172338) : const Color(0xFFF4F5F9),
...
class _TextChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: dark ? const Color(0xFF172338) : const Color(0xFFF2F3F7),
        borderRadius: BorderRadius.circular(999),
      ),
      child: Text(
        text,
        style: TextStyle(
          color: dark ? const Color(0xFFF5F7FF) : AppColors.navy,
          fontSize: 12,
          fontWeight: FontWeight.w700,
        ),
      ),
