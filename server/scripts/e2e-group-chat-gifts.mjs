...
async function main() {
  ...
  if (!freeGift || Number(freeGift.coinCost) !== 0 || Number(freeGift.profileXp) !== 1 || freeGift.dailyFree !== true || freeGift.name !== 'Selam') {
    throw new Error(`Free Selam gift catalog entry mismatch: ${JSON.stringify(freeGift)}`);
  }
  if (catalog.gifts.some((gift) => gift.code === 'free_wave')) {
    throw new Error('Legacy free_wave gift must not be active.');
  }
  ...
  if (Number(sent.wallet?.profileXp) !== 3) {
    throw new Error(`Sender profile XP should gain 3 from paid coffee-code gift: ${JSON.stringify(sent.wallet)}`);
  }
  ...
  if (Number(sent.recipientSummary?.profileXp) !== 3) {
    throw new Error(`Recipient profile XP should gain 3 from paid coffee-code gift: ${JSON.stringify(sent.recipientSummary)}`);
  }
  ...
  if (Number(afterFree.wallet?.profileXp) !== 6) {
    throw new Error(`Paid gift + three free gifts should total 6 sender profile XP: ${JSON.stringify(afterFree.wallet)}`);
  }
  ...
  console.log('Rules: paid gifts + Selam as 3/day free gift + tiny free XP + profile XP + idempotency + no self/closed/insufficient gift + matching unaffected');
