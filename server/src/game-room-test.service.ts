...
export class GameRoomTestService {
  ...
  private static readonly WRITE_MS = 45_000;
  private static readonly VOTE_MS = 5_000;
  private static readonly RESULT_MS = 10_000;
  private static readonly ROUND_MS =
    GameRoomTestService.WRITE_MS + GameRoomTestService.VOTE_MS + GameRoomTestService.RESULT_MS;
  ...
  private async resolve(state: State) {
    ...
    for (const correctUserId of correct) {
      ...
      await this.infra.db.query(
        `insert into user_wallets(user_id,profile_xp) values($1,20)
         on conflict(user_id) do update set profile_xp=user_wallets.profile_xp+20, updated_at=now()`,
        [correctUserId],
      );
    }
  }
  ...
  private async completeTimedRound(state: State) {
    if (state.round.statements.length !== 3) {
      state.round.statements = this.botStatements(state.roundIndex);
  ...
  private pairCompatibility(state: State, a: string, b: string) {
    const shared = state.completedRounds.filter(
      (round) => round.ownerUserId !== a && round.ownerUserId !== b && round.votes[a] !== undefined && round.votes[b] !== undefined,
    );
    if (!shared.length) return 0;
    const sameAnswers = shared.filter((round) => round.votes[a] === round.votes[b]).length / shared.length;
    const sameCorrectness = shared.filter(
      (round) => (round.votes[a] === round.lieIndex) === (round.votes[b] === round.lieIndex),
  ...
  private async buildSuggestions(state: State) {
    ...
    const mutual = pairs.filter((pair) => {
      const aBest = bestFor.get(pair.a), bBest = bestFor.get(pair.b);
      return aBest != null && bBest != null && other(aBest, pair.a) === pair.b && other(bBest, pair.b) === pair.a;
    });
    for (const pair of mutual) {
      if (assigned.has(pair.a) || assigned.has(pair.b)) continue;
      chosen.push(pair); assigned.add(pair.a); assigned.add(pair.b);
    }
    for (const pair of pairs) {
      if (assigned.has(pair.a) || assigned.has(pair.b)) continue;
      chosen.push(pair); assigned.add(pair.a); assigned.add(pair.b);
    ...
    if (human) {
      ...
      if (bot && state.suggestions[bot.id]?.partnerUserId === human.id) state.finalChoices[bot.id] = 'match';
    }
  }

  private async syncTimeline(state: State) {
    if (state.finished) return;
    const elapsed = Math.max(0, Date.now() - state.startedAtMs);
    const targetRound = Math.floor(elapsed / GameRoomTestService.ROUND_MS);

    while (!state.finished && state.roundIndex < Math.min(targetRound, GameRoomTestService.TOTAL_ROUNDS)) {
      await this.completeTimedRound(state);
      if (state.roundIndex >= GameRoomTestService.TOTAL_ROUNDS - 1) {
        state.finished = true;
    ...
    if (targetRound >= GameRoomTestService.TOTAL_ROUNDS) {
      if (!state.finished) {
        await this.completeTimedRound(state);
        state.finished = true;
        await this.buildSuggestions(state);
    ...
    const inRound = elapsed % GameRoomTestService.ROUND_MS;
    if (inRound >= GameRoomTestService.WRITE_MS && state.round.statements.length !== 3) {
      state.round.statements = this.botStatements(state.roundIndex);
      state.round.lieIndex = 2;
    }
    if (inRound >= GameRoomTestService.WRITE_MS + GameRoomTestService.VOTE_MS && !state.round.resolved) {
      await this.completeTimedRound(state);
    }
  }
  ...
  private view(state: State, userId: string) {
    ...
    const elapsed = Math.max(0, Date.now() - state.startedAtMs);
    const inRound = elapsed % GameRoomTestService.ROUND_MS;
    let phase: 'write' | 'vote' | 'result' | 'final' = 'write';
    let phaseEndsAtMs = state.startedAtMs + state.roundIndex * GameRoomTestService.ROUND_MS + GameRoomTestService.WRITE_MS;
    if (state.finished) {
      phase = 'final';
      phaseEndsAtMs = Date.now();
    } else if (inRound >= GameRoomTestService.WRITE_MS + GameRoomTestService.VOTE_MS) {
      phase = 'result';
      phaseEndsAtMs = state.startedAtMs + (state.roundIndex + 1) * GameRoomTestService.ROUND_MS;
    } else if (inRound >= GameRoomTestService.WRITE_MS) {
      phase = 'vote';
      phaseEndsAtMs = state.startedAtMs + state.roundIndex * GameRoomTestService.ROUND_MS + GameRoomTestService.WRITE_MS + GameRoomTestService.VOTE_MS;
    }
    ...
    return {
      ...
      totalRounds: GameRoomTestService.TOTAL_ROUNDS,
      phase,
      phaseEndsAt: new Date(phaseEndsAtMs).toISOString(),
      gameStartedAt: new Date(state.startedAtMs).toISOString(),
  ...
  async create(userId: string) {
    ...
    try {
      ...
      const profiles = await client.query<{ user_id: string; name: string; gender: string; looking_for: string; photo_url: string }>(
        `select u.id::text user_id, coalesce(nullif(trim(p.display_name),''),'Oyuncu') name,
                coalesce(p.gender,'') gender, coalesce(mp.looking_for,'Herkes') looking_for,
      ...
      const players = memberIds.map((id) => {
        ...
        return { id, name: row.name, test: id !== String(userId), gender: row.gender, lookingFor: row.looking_for, photoUrl: row.photo_url };
      });
      const startedAtMs = created.rows[0]?.started_at?.getTime() ?? Date.now();
      const state: State = {
        roomId,
        players,
        roundIndex: 0,
  ...
  async submitStatements(userId: string, roomId: string, statements: unknown, lieIndex: unknown) {
    ...
    const elapsed = (Date.now() - state.startedAtMs) % GameRoomTestService.ROUND_MS;
    if (elapsed >= GameRoomTestService.WRITE_MS) throw new BadRequestException('Yazma süresi kapandı.');
    ...
    if (choice === 'match') {
      ...
      if (state.finalChoices[partnerId] === 'match' && state.suggestions[partnerId]?.partnerUserId === String(userId)) {
        const inserted = await this.infra.db.query<{ id: string }>(
          `insert into matches(user_a_id,user_b_id,source_room_id) values($1,$2,$3)
        ...
        if (matchId) {
          ...
          await this.infra.db.query(
            ...
            [userId, partnerId, `${suggestion.partnerName} ile oyun uyumunuz karşılıklı eşleşti.`, matchId, `${state.players.find((p) => p.id === String(userId))?.name ?? 'Bir oyuncu'} ile oyun uyumunuz karşılıklı eşleşti.`],
          );
        }
      }
    }
  ...
  async vote(userId: string, roomId: string, choice: unknown) {
    ...
    if (state.finished) throw new BadRequestException('Oyun tamamlandı.');
    const elapsed = (Date.now() - state.startedAtMs) % GameRoomTestService.ROUND_MS;
    if (elapsed < GameRoomTestService.WRITE_MS || elapsed >= GameRoomTestService.WRITE_MS + GameRoomTestService.VOTE_MS) {
      throw new BadRequestException('Oylama süresi açık değil.');
    }
