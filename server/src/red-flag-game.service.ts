...
export class RedFlagGameService {
  ...
  async finalChoice(userId:string,roomId:string,raw:unknown){const s=this.games.get(roomId);if(!s)throw new BadRequestException('Oyun durumu bulunamadı.');if(!s.players.some((p)=>p.id===String(userId)))throw new ForbiddenException('Bu Red Flag / Green Flag oyununa erişimin yok.');await this.syncTimeline(s);if(s.phase!=='final')throw new BadRequestException('Oyun henüz tamamlanmadı.');const c=String(raw??'');if(c!=='match'&&c!=='continue')throw new BadRequestException('Geçerli final seçimi gönder.');const rec=s.suggestions[String(userId)];if(!rec)throw new BadRequestException('Uygun eşleşme önerisi bulunamadı.');s.finalChoices[String(userId)]=c;
